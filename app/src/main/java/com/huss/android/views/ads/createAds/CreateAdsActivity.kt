...
import com.huss.android.viewModels.ads.CreateAdViewModel
import com.huss.android.viewModels.ads.UpdateAdViewModel
...
class CreateAdsActivity : AppCompatActivity(), BottomNavPay.PayState, NetworkReceiverUtil.ConnectivityReceiverListener {
    ...
    private fun popSpinnerCatAndSub(categoryName: String, subCategoryName: String) {
        ...
        categories?.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                val subCategoryViewModel = ViewModelProviders.of(this@CreateAdsActivity).get(SubCategoryViewModel::class.java)
                subCategoryViewModel.init(token, categories.getItemAtPosition(position).toString())
                subCategoryViewModel.subCategory.observe(this@CreateAdsActivity, Observer<SubCategory?> { subCatResponse ->
                    for (item in subCatResponse?.data!!) {
                        sublist += item!!.name
                    }
                    val subAdapter = ArrayAdapter<String>(this@CreateAdsActivity, android.R.layout.simple_spinner_item,
                            sublist)
                    ...
                    for ((i, image) in arrayList.withIndex()) {
                        val uploadCallBack = object : UploadCallback {
                            override fun onSuccess(requestId: String?, resultData: MutableMap<Any?, Any?>?) {
                                val createImageViewModel = ViewModelProviders.of(this@CreateAdsActivity).get(CreateImageViewModel::class.java)
                                ...
                                createImageViewModel.createImageResponse
                                        .observe(this@CreateAdsActivity, Observer<Image> {
                                            progress.dismiss()
                                            val progressSuccess = AlertDialog.Builder(this@CreateAdsActivity)
                                            ...
                                            progressSuccess.setPositiveButton("Ok") { dialog, _ ->
                                                dialog.dismiss()
                                                startActivity(Intent(this@CreateAdsActivity, ProfileActivity::class.java))
                                            }
...
//                                progressSuccess.setPositiveButton("Ok") { dialog, _ ->
//                                    dialog.dismiss()
//                                    startActivity(Intent(this@CreateAds, ProfileActivity::class.java))
//                                }
//                                progressSuccess.show()
//
//
...
//            }
                    ...
                    if (!state) {
                        ...
                        if (intent.extras == null) {
                            val createAdViewModel = ViewModelProviders.of(this@CreateAdsActivity).get(CreateAdViewModel::class.java)
                            createAdViewModel.init(ads, token)
                            createAdViewModel.createResponse.observe(this@CreateAdsActivity, Observer<Ads> { productId ->
                                /*Update phone number in profile */
                                val profileViewModel = ViewModelProviders.of(this@CreateAdsActivity).get(ProfileViewModel::class.java)
                                val profile = com.huss.android.models.Profile().Data()
                                profile.token = token
                                profile.id = userId
...
//                                profile.city = sharedPref.getString(LOCATION, "")
                                ...
                                profileViewModel.updateProfile().observe(this@CreateAdsActivity, Observer {
                            ...
                            val updateAdViewModel = ViewModelProviders.of(this@CreateAdsActivity).get(UpdateAdViewModel::class.java)
                            updateAdViewModel.init(token, singleAd)
                            updateAdViewModel.updateAdResponse.observe(this@CreateAdsActivity, Observer<SingleAd> {
                                progress.dismiss()
                                val progressSuccess = AlertDialog.Builder(this@CreateAdsActivity)
                                progressSuccess.setCancelable(true)
                                progressSuccess.setTitle("Congratulations!")
...
//            registerReceiver(network, filter)
    ...
    private fun showSnack() {
        val snackbar = Snackbar.make(postAd, "No internet connection", Snackbar.LENGTH_INDEFINITE)
        snackbar.setAction("Ok") { snackbar.dismiss() }
